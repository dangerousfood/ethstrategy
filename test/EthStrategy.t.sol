...
contract EthStrategyGovernorTest is BaseTest {
    ...
    function test_constructor_success() public {
        ...
        ethStrategy = new EthStrategy(defaultTimelockDelay, defaultQuorumPercentage, defaultVoteExtension, defaultVotingDelay, defaultVotingPeriod, defaultProposalThreshold);
    ...
    function test_castVote_success() public {
        ...
        assertEq(ethStrategy.proposalSupport(proposalId, address(alice)), uint8(GovernorCountingSimple.VoteType.For), "alice has voted");
        assertEq(ethStrategy.proposalSupport(proposalId, address(bob)), uint8(GovernorCountingSimple.VoteType.Against), "bob has voted");
        assertEq(ethStrategy.proposalSupport(proposalId, address(charlie)), uint8(GovernorCountingSimple.VoteType.For), "charlie has voted");
    }
    ...
    function test_executeOperations_success() public {
        ...
        (uint256 proposalId, address[] memory targets, uint256[] memory values, bytes[] memory calldatas, string memory description) = setupDefaultDutchAuctionProposal();
        vm.prank(alice);
        ethStrategy.castVote(proposalId, uint8(GovernorCountingSimple.VoteType.For));
        vm.prank(bob);
        ethStrategy.castVote(proposalId, uint8(GovernorCountingSimple.VoteType.For));
    ...
    function test_rageQuit_paused_success() public {
        ...
        assertEq(usdcToken.balanceOf(address(ethStrategy)), defaultAmount / 2, "ethStrategy's usdc balance not assigned correctly");
    ...
    function test_rageQuit_votedAgainst_success() public {
        ...
        assertEq(usdcToken.balanceOf(address(ethStrategy)), defaultAmount * 3 / 4, "ethStrategy's usdc balance not assigned correctly");
        assertEq(alice.balance, defaultAmount / 4, "alice's eth balance not assigned correctly");
        assertEq(address(ethStrategy).balance, defaultAmount * 3 / 4, "ethStrategy's eth balance not assigned correctly");
        assertEq(ethStrategy.balanceOf(bob), defaultProposerAmount, "bob's ethStrategy balance not assigned correctly");
        assertEq(ethStrategy.balanceOf(charlie), defaultProposerAmount, "charlie's ethStrategy balance not assigned correctly");
        assertEq(ethStrategy.balanceOf(proposer), defaultProposerAmount, "proposer's ethStrategy balance not assigned correctly");
    ...
    function test_proposal_success() public {
        ...
        vm.warp(block.timestamp + ethStrategy.votingPeriod() + 1);
        assertEq(
            uint8(ethStrategy.state(proposalId)), uint8(IGovernor.ProposalState.Queued), "proposal not succeeded"
        );
    ...
    function test_proposalDeadline_success() public {
        (uint256 proposalId, , , , ) = setupDefaultDutchAuctionProposal();
        assertEq(ethStrategy.proposalDeadline(proposalId), block.timestamp + ethStrategy.votingPeriod() - 1, "proposal deadline not assigned correctly");
    }
    ...
    function test_rageQuit_revert_ERC20InsufficientBalance() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IERC20Errors.ERC20InsufficientBalance.selector, alice, defaultProposerAmount - 1, defaultProposerAmount));
        ethStrategy.rageQuit(defaultProposerAmount, proposalId, new address[](1));
    }

    function setupDefaultDutchAuctionProposal() public returns (uint256 proposalId, address[] memory targets, uint256[] memory values, bytes[] memory calldatas, string memory description) {
